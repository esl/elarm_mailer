...
-module(elarm_mailer_test_smtp_server).
...
-define(LOG(Fmt,Args), noop(Fmt, Args)).
-define(LOG(Fmt), noop(Fmt)).
...
init(Hostname, SessionCount, Address, Options) ->
	...
	case SessionCount > 20 of
		false ->
			...
			{ok, Banner, State};
		true ->
			?LOG("Connection limit exceeded~n"),
...
handle_MAIL_extension(<<"X-SomeExtension">> = Extension, State) ->
	?LOG("Mail from extension ~s~n", [Extension]),
	% any MAIL extensions can be handled here
	{ok, State};
handle_MAIL_extension(Extension, _State) ->
	?LOG("Unknown MAIL FROM extension ~s~n", [Extension]),
	error.
...
handle_RCPT_extension(<<"X-SomeExtension">> = Extension, State) ->
	...
	?LOG("Mail to extension ~s~n", [Extension]),
	{ok, State};
handle_RCPT_extension(Extension, _State) ->
	?LOG("Unknown RCPT TO extension ~s~n", [Extension]),
	error.
...
notify_mailbox_or_crash(Msg) ->
    case erlang:whereis(?MAILBOX_PROCESS) of
        undefined -> error(mailbox_process_not_responding);
        Pid -> Pid ! Msg
    end.
